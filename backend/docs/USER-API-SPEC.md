...
- 최근 5분 내 비밀번호 인증 후 `user` 행을 SQL DELETE로 삭제한다.
- 설정·기기·동의·갱신 토큰·친구 관계/요청·본인이 관련된 여행 초대·중복 요청 응답 기록도 삭제한다.
- 다른 멤버가 있는 ACTIVE 여행의 여행장은 먼저 보관해야 한다 (`409 ACTIVE_TRIP_OWNER`).
- 여행은 보관하고 `owner_id`를 NULL로 해제한다. 다른 멤버의 여행과 채팅은 유지하며 채팅 발신자 연결은 DB의 SET NULL 정책을 따른다.
- 모두 같은 트랜잭션으로 처리한다. 기존 토큰은 사용할 수 없고 동일 이메일로 재가입할 수 있다.
- 기존 WITHDRAWN 계정의 일괄 삭제는 수행하지 않는다.
...
| 12 | 탈퇴 처리 | 물리 삭제. 연결된 개인 데이터 삭제 및 같은 이메일 재가입 허용 |
