...
- frontend: typecheck, 34 tests, web/Android/iOS export 통과. 이전 계정 요청의 늦은 응답이 새 세션을 덮어쓰거나 종료하지 않는 회귀 검사 포함.
- 실제 MySQL/API: 테스트 전용 계정으로 동시 재발급 200/401, 고정 만료 시각, 재사용 시 후속 토큰 무효화, 로그아웃 후 재발급 401 확인.
- 브라우저: 실제 로그인, 새로고침 후 동일 계정 복원, 로그아웃 및 이후 새로고침에서도 로그인 화면 유지 확인.
- SecureStore 실기기 검증은 수행하지 않았다. 네이티브 export는 APK/IPA 설치 검증이 아니다.
- 상세 계약: [토큰 재발급 구현](../../docs/AUTH-REFRESH-2026-09-21.md).
