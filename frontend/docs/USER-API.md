...
기기 등록/삭제는 UserApi에 메서드가 있지만 FCM 발급 연결이 없어 UI가 호출하지 않는다. 임의 토큰을 만들어 등록하지 않는다. 설정 스위치는 서버의 동의 설정만 저장하며 OS 권한 요청이나 실제 전송을 시작하지 않는다.

가입은 필수 약관 두 종류와 선택 마케팅을 명시적으로 제출한다. 버전은 현재 서버 기본값 1.0이다. 약관 전문/운영 정책은 별도 확정이 필요하다. 가입 응답에는 토큰이 없으므로 성공 후 로그인 화면으로 전환한다.

계좌 입력은 저장 후 지우고 서버가 반환한 마스킹 값만 표시한다. 수정 요청에서 생략은 유지, null은 삭제다. 비밀번호·JWT·계좌 원문은 로컬 여행 저장소에 기록하지 않는다. refresh/자동 로그인은 미구현이다. 보호 API의 401 및 ACCOUNT_UNAVAILABLE는 로그인 화면으로 돌려보낸다.
